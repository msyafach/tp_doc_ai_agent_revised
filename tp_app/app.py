...
with st.sidebar:
    ...
    st.download_button(
        ...
        file_name=f"tp_project_{_co}_{_fy}.json",
...
# ─── Step 10: Run AI Agents ──────────────────────────────────────────────────
    ...
    |---------|-------|--------|
    | **Business Activities** | 🧠 LLM Analysis | LLM (auto if field empty) |
    ...
    | Functional Analysis | 🧠 LLM Analysis | LLM (from your inputs) |
    | Business Characterization | 🧠 LLM Analysis | LLM (from functional analysis) |
    | Comparability Analysis | 🧠 LLM Analysis | LLM (from your inputs) |
    | Method Selection Justification | 🧠 LLM Analysis | LLM (from characterization) |
    | PLI Selection Rationale | 🧠 LLM Analysis | LLM (from method) |
    | Conclusion | 🧠 LLM Analysis | LLM (from all data) |
    | Executive Summary | 🧠 LLM Analysis | LLM (from everything) |
    """)
    ...
    if st.session_state.agent_ran:
        ...
        sections_to_review = [
            ...
            # "business_characterization_text" removed — now filled manually in Step 4
            ...
            with st.expander(f"🤖 {label}", expanded=False):
                ...
                if st.button(f"🔄 Regenerate", key=f"regen_{key}"):
                    try:
                        ...
                        result = run_single_agent(agent_map[key], agent_state)
                        if key in result:
                            st.session_state[key] = result[key]
                            st.rerun()
                    except Exception as e:
...
# ─── Step 11: Review & Export ─────────────────────────────────────────────────
        ...
        if st.button("📄 Export Full Document (.docx)", type="primary", use_container_width=True):
            with st.spinner("Generating document..."):
                try:
                    ...
                    output_filename = (
                        f"TP_LocalFile_{st.session_state.company_short_name}"
                        f"_{st.session_state.fiscal_year}.docx"
                    )
        ...
        if st.button("📋 Export from Master Template (.docx)", use_container_width=True):
            with st.spinner("Rendering Jinja2 template..."):
                try:
                    ...
                    output_filename = (
                        f"TP_LocalFile_{st.session_state.company_short_name}"
                        f"_{st.session_state.fiscal_year}_template.docx"
                    )
                    with tempfile.NamedTemporaryFile(suffix=".docx", delete=False) as tmp:
                        output_path = tmp.name
    ...
    with col1:
        ...
        st.download_button(
            ...
            file_name=f"tp_project_{st.session_state.company_short_name}_{st.session_state.fiscal_year}.json",
            mime="application/json",
            use_container_width=True,
        )
